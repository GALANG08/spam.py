...
class nyepam:
	...
	def mulai(self):
		...
		except reek.exceptions.ReadTimeout:exit("\x1b[0;31mSinyal Lu Tidak Stabil")
		except reek.exceptions.ConnectionError:exit("\x1b[0;31mSinyal Lu Error")
		except (KeyboardInterrupt,EOFError):exit("[!] Exit")
__import__("os").system("clear")
os.system("xdg-open https://youtube.com/channel/UC7B2WnnlAjsKfuKeCO83oYA")
mengetik (f"""
{sp}{u} [{H}×{u}]{k}Author {H}: {k}MR•GALANG
{sp}{u} [{H}×{u}]{k}Team   {H}: {k} Hacker 7A
{sp}{u} [{H}×{u}]{k}Wa     {H}: {k}+62 831-1196-2790
{sp}{u} [{H}×{u}]{k}Yt     {H}: {k}GALANG ID
{u}╔═══╗╔═══╗╔═══╗╔═╗╔═╗  {u}╔═══╗╔═╗╔═╗╔═══╗
{u}║╔═╗║║╔═╗║║╔═╗║║║╚╝║║  {u}║╔═╗║║║╚╝║║║╔═╗║
{u}║╚══╗║╚═╝║║║─║║║╔╗╔╗║  {u}║╚══╗║╔╗╔╗║║╚══╗
{u}╚══╗║║╔══╝║╚═╝║║║║║║║  {u}╚══╗║║║║║║║╚══╗║
...
{k}     [ {H}Gak untuk salah gunakan Kampang {k}]
{H}     ═══════════════════════════\n
""")
while True:
	try:
